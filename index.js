...
+ 'FN:Mr.cangrejo\n' // Nama
+ 'ORG:Mr.Cangrejo_Bot;\n' // Nama bot
+ 'TEL;type=CELL;type=VOICE;waid=573184581139:+57 31 8458 1139\n' // Nomor bot
+ 'END:VCARD' 
...
Bienvenid@ a 
*${mdata.subject}*
Jode que bueno verte por aca 🤩Te damos la bienvenida a este amigable grupo 🥺, echa un vistaso a las reglas para que no tengan que sacarte 😉 preséntate porfavor con 
*Nombre* :*Edad* : y *Género* Gracias att: el botsito🤖
...
Usa ${prefix}verify para verificarte y poder usar el bot.`
				...
				num = anu.participants[0]
				teks = `Que bueno que te fuiste ala verga😈  @${num.split('@')[0]} ya no te queriamos ojala nunca vuelvas a este grupo👋`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
〘 *BOT* 〙
‣ Prefijo:  *「 ${prefix} 」*
‣ Yo soy: 🦀⃢⃝✞⃝🇨🇴𝙘𝙖𝙣𝙜𝙧𝙚𝙟𝙞𝙩𝙤🦀⃢⃝✞🏴‍☠️𝔟𝔬𝔱
‣ Conección: *Baileys*
...
//--mengeluarkan member
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Se ha eliminado a un mongolico: @${mentioned[0].split('@')[0]}`, mentioned, true)
						Lxa.groupRemove(from, mentioned)
					}
					break
...
//--list admin grup
				case 'listadmins':
				  case 'listadmin':
				    case 'adminlist':
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista admin de admins facheros facheritos *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
...
case 'closegc':
  ...
  const close = {
text: `Grupo cerrado por @${nomor.split("@s.whatsapp.net")[0]}\n ahora solo los admins facheros facheritos pueden escribir`,
contextInfo: {
  mentionedJid: [nomor]
}
  }
